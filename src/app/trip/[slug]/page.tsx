...
type Media = { id: string; uploader_id: string; storage_path: string; caption: string | null; kind: "photo" | "video"; captured_at: string | null; created_at: string; place_name: string | null; latitude: number | null; longitude: number | null };
...
export default async function TripPage({ params }: PageProps<"/trip/[slug]">) {
  ...
  const [{ data: checkinData }, { data: mediaData }, { data: memberData }, { data: userData }, { data: reactionData }] = await Promise.all([
    ...
    supabase.from("media").select("id,uploader_id,storage_path,caption,kind,captured_at,created_at,place_name,latitude,longitude").eq("trip_id", trip.id).order("captured_at", { ascending: false, nullsFirst: false }),
  ...
  const signedMedia = media.length ? await supabase.storage.from("trip-media").createSignedUrls(media.map((item) => item.storage_path), 3600) : { data: [] };
  const mediaWithUrls = media.map((item, index) => ({ ...item, url: signedMedia.data?.[index]?.signedUrl ?? null }));
  ...
  const canContribute = Boolean(userData.user && memberData?.some((member) => member.user_id === userData.user.id));
  ...
  return <main className="journey-page">
    <header className="site-header journey-header"><Link className="brand" href="/"><span className="brand-mark" aria-hidden="true"><span /><span /><span /></span>Roamline</Link><div className="header-actions"><ThemeToggle />{userId === trip.owner_id ? <InvitePeople tripId={trip.id} userId={userId} /> : null}<ShareButton title={`${trip.name} on Roamline`} /></div></header>
    <section className="journey-intro page-shell"><div><p className="journey-overline">{trip.status === "published" ? "SHARED JOURNEY" : trip.status.toUpperCase()}</p><div className="journey-title-row"><div className="journey-title-main"><h1>{trip.name}</h1>{userId === trip.owner_id ? <Link className="trip-title-edit" href={`/trip/${trip.slug}/edit`} aria-label="Edit trip settings" title="Edit trip"><Pencil size={13} /></Link> : null}<TripStory title={trip.name} moments={storyMoments} /></div></div><p className="journey-description">{trip.description || "A new journey is about to begin."}</p><p className="trip-date-line"><CalendarDays size={14} /> {tripDates(trip.start_date, trip.end_date)}</p></div><div className="journey-people"><div className="avatar-stack" aria-label={travelers.map((traveler) => traveler.display_name).join(", ")}>{travelers.map((traveler, index) => <span key={traveler.id} title={traveler.display_name} style={{ backgroundColor: avatarColors[index] }}>{initials(traveler.display_name)}</span>)}</div><p>{travelers.length === 1 ? "1 traveler" : `${travelers.length} travelers`}</p></div></section>
    ...
    <MediaSelectionProvider slug={trip.slug}><section className="timeline page-shell real-timeline">
      {timelineDays.length === 0 ? <div className="timeline-empty"><span><MapPin size={20} /></span><h2>No moments yet</h2><p>Photos, videos, and check-ins will appear here in chronological order.</p></div> : <><div className="timeline-rail" aria-hidden="true"><span /></div>{timelineDays.map((day) => <section className="timeline-day" key={timelineDateKey(day.date)}>
        ...
        {day.checkins.map((checkin) => <CheckinCard key={checkin.id} slug={trip.slug} canManage={Boolean(userId && (userId === trip.owner_id || userId === checkin.author_id))} checkin={{ id: checkin.id, placeId: checkin.place_id, placeName: checkin.place_name, address: checkin.formatted_address, note: checkin.note, occurredAt: checkin.occurred_at, latitude: checkin.latitude, longitude: checkin.longitude }} />)}
        {day.media.length > 0 ? <MediaGallery slug={trip.slug} userId={userId} returnTo={`/trip/${trip.slug}`} items={day.media.filter((item): item is typeof item & { url: string } => Boolean(item.url)).map((item) => ({ id: item.id, storagePath: item.storage_path, url: item.url, caption: item.caption, kind: item.kind, capturedAt: item.captured_at, placeName: item.place_name, latitude: item.latitude, longitude: item.longitude, reactions: reactionsByMedia.get(item.id) ?? [], canManage: Boolean(userId && (userId === trip.owner_id || userId === item.uploader_id)) }))} /> : null}
      </section>)}</>}
