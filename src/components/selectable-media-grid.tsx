...
export function SelectableMediaGrid({ items, onOpen }: { items: SelectableMediaItem[]; onOpen: (index: number) => void }) {
  ...
  return <div className={`moment-grid real-moments${selecting ? " is-selecting" : ""}`}>{items.map((item, index) => <div className={`moment media-open${selected.has(item.id) ? " is-selected" : ""}`} key={item.id}>
    {item.kind === "photo" || item.thumbnailStoragePath ? <Image src={item.thumbnailUrl} alt={item.caption || (item.kind === "photo" ? "Trip photo" : "Trip video")} fill sizes="(max-width: 800px) 50vw, 33vw" /> : <video src={item.url} muted preload="metadata" />}{item.caption ? <span className="media-caption">{item.caption}</span> : null}
