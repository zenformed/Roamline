...
import { PlacePreviewDialog } from "@/components/place-preview-dialog";
...
export function CheckinCard({ checkin, canManage, slug, postStyle = false, authorId, authorName = "Traveler", authorColor = "#dce8ff", travelers = [], attendeeIds = [] }: { checkin: Checkin; canManage: boolean; slug: string; postStyle?: boolean; authorId: string; authorName?: string; authorColor?: string; travelers?: Traveler[]; attendeeIds?: string[] }) {
  ...
  const [previewingPlace, setPreviewingPlace] = useState(false);
  ...
  return <>
    <article className={`checkin-card${postStyle ? " post-checkin" : ""}${canManage ? " can-manage" : ""}${selected.has(checkin.id) ? " is-selected" : ""}`} id={`checkin-${checkin.id}`} onContextMenu={(event) => { if (canManage && isMobile()) event.preventDefault(); }} onPointerDown={beginPress} onPointerUp={endPress} onPointerCancel={endPress} onPointerLeave={endPress} onClick={activateCard}>
      {postStyle ? <div className="post-checkin-main"><span className="post-avatar" style={{ backgroundColor: authorColor }}>{initials(authorName)}</span><div><p className="post-byline"><strong>{authorName}</strong>{companionNames.length ? <> is with <strong>{companionNames[0]}</strong>{companionNames.length > 1 ? <> and <span title={companionNames.slice(1).join(", ")}>{companionNames.length - 1} {companionNames.length === 2 ? "other" : "others"}</span></> : null} at </> : <> is at </>}<button type="button" onClick={() => setPreviewingPlace(true)}>{checkin.placeName}</button></p><time>{new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }).format(new Date(checkin.occurredAt))}</time>{checkin.note ? <p className="post-checkin-note">{checkin.note}</p> : null}</div></div> : <div className="checkin-main"><button className="place-thumbnail-button" type="button" aria-label={`Preview ${checkin.placeName}`} title="Preview place" onClick={() => setPreviewingPlace(true)}><PlaceThumbnail placeId={checkin.placeId} name={checkin.placeName} /></button><div className="checkin-copy"><span className="moment-time">{new Intl.DateTimeFormat("en-US", { hour: "numeric", minute: "2-digit" }).format(new Date(checkin.occurredAt))} · CHECK-IN</span><h3>{checkin.placeName}</h3>{checkin.note ? <p>{checkin.note}</p> : null}</div></div>}
    ...
    <PlacePreviewDialog placeId={checkin.placeId} name={checkin.placeName} latitude={checkin.latitude} longitude={checkin.longitude} open={previewingPlace} onClose={() => setPreviewingPlace(false)} />
