...
export function HeaderNavigation({ signedIn, displayName, searchable = false }: { signedIn: boolean; displayName?: string | null; searchable?: boolean }) {
  ...
  return <>
    ...
    {signedIn ? <div className="header-menu" ref={menuRef}><button className="header-icon-button" type="button" aria-label="Open account menu" aria-expanded={menuOpen} onClick={() => setMenuOpen((value) => !value)}><Menu size={19} /></button>{menuOpen ? <div className="header-menu-popover"><span>{displayName || "Your account"}</span><Link href="/?scope=mine">Your trips</Link><Link href="/">All trips</Link><form action={signOut}><button type="submit">Sign out</button></form></div> : null}</div> : <Link className="text-button" href="/login">Sign in</Link>}
  </>;
}
