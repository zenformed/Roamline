...
- [~] A camera-only floating action beside Add moment opens the device's native rear camera and sends the photo into the normal date/caption/publishing flow. On camera return it isolates Android's GPU-composited map/header layers and renders a generated 104px preview instead of painting the full-resolution camera JPEG as a thumbnail; physical Android retesting remains.
...
- [~] New video uploads are locally compressed to a smaller playable MP4 when beneficial and attempt a 480px WebP poster; preview extraction is time-bounded and optional so it cannot block publishing. Timeline grids use the poster when available; a physical iPhone video upload pass remains.
